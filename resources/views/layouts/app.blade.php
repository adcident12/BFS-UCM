...
<head>
    ...
    <style>
        ...
        .nav-section { overflow: hidden; transition: max-height 0.25s ease; }
...
<body class="bg-slate-50 min-h-screen font-sans">
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        {{-- ─── Sidebar ─────────────────────────────────────────────────── --}}
        <aside id="sidebar"
               class="w-64 bg-[#0f172a] text-white flex flex-col flex-shrink-0
            ...
            <nav class="flex-1 px-3 py-4 overflow-y-auto overflow-x-hidden">
                ...
                <button type="button" onclick="navToggle('main')"
                        class="w-full flex items-center justify-between px-3 py-1.5 mb-1 text-[10px] font-semibold text-slate-500 uppercase tracking-widest hover:text-slate-300 transition-colors">
                    <span>เมนูหลัก</span>
                    <svg id="chevron-main" class="nav-chevron w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7"/>
                    </svg>
                </button>
                <div id="section-main" class="nav-section space-y-0.5" style="max-height:500px">
                    @php
                        $navItems = [
                ...
                <button type="button" onclick="navToggle('admin')"
                    ...
                    <svg id="chevron-admin" class="nav-chevron w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7"/>
                    </svg>
                </button>
                <div id="section-admin" class="nav-section space-y-0.5" style="max-height:500px">
                ...
                <button type="button" onclick="navToggle('docs')"
                        class="w-full flex items-center justify-between px-3 py-1.5 mb-1 text-[10px] font-semibold text-slate-500 uppercase tracking-widest hover:text-slate-300 transition-colors">
                    <span>คู่มือ</span>
                    <svg id="chevron-docs" class="nav-chevron w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7"/>
                    </svg>
                </button>
                <div id="section-docs" class="nav-section space-y-0.5" style="max-height:500px">
                ...
                <button type="button" onclick="navToggle('dev')"
                        class="w-full flex items-center justify-between px-3 py-1.5 mb-1 text-[10px] font-semibold text-slate-500 uppercase tracking-widest hover:text-slate-300 transition-colors">
                    <span>นักพัฒนา</span>
                    <svg id="chevron-dev" class="nav-chevron w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7"/>
                    </svg>
                </button>
                <div id="section-dev" class="nav-section space-y-0.5" style="max-height:500px">
                    @php
                ...
                {{-- ── บัญชี ── --}}
                <div class="h-px bg-white/5 mx-3 my-3"></div>
                <p class="px-3 mb-1 text-[10px] font-semibold text-slate-500 uppercase tracking-widest">บัญชี</p>
        ...
        {{-- ─── Main Content ─────────────────────────────────────────────── --}}
        <div class="flex-1 flex flex-col min-w-0 lg:pl-64 overflow-hidden">

            {{-- Sticky Top bar --}}
            <header class="sticky top-0 z-20 bg-white/80 backdrop-blur-md border-b border-slate-100/80 px-4 md:px-8 py-3.5 flex items-center justify-between shadow-sm shadow-slate-100/50 flex-shrink-0">
...
(function () {
    ...
    function expand(id) {
        ...
        try { localStorage.setItem('nav-' + id, '1'); } catch(e) {}
    }
    ...
    function collapse(id) {
        ...
        try { localStorage.setItem('nav-' + id, '0'); } catch(e) {}
    }

    window.navToggle = function (id) {
        var el = document.getElementById('section-' + id);
        if (!el) return;
        if (el.style.maxHeight === '0px') expand(id);
        else collapse(id);
    };

    // Init: restore state from localStorage; always expand active section
    SECTIONS.forEach(function (id) {
        ...
        var saved = '';
        try { saved = localStorage.getItem('nav-' + id); } catch(e) {}
        if (saved === '0') collapse(id);
        else expand(id); // default open
    });
})();
</script>
