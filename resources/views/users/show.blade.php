...
<div class="mb-6 relative overflow-hidden rounded-2xl"
    ...
    <div class="relative px-6 md:px-8 py-6 flex flex-col sm:flex-row items-start gap-4 md:gap-5">
        {{-- Avatar --}}
        ...
        <div class="flex-1 min-w-0">
            ...
            <div class="flex flex-wrap items-center gap-2 mt-2">
                ...
                @if ($user->email)
                    <span class="inline-flex items-center gap-1 text-xs text-slate-400 font-medium">
                        <svg class="w-3 h-3 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                        {{ $user->email }}
                    </span>
                @endif
                @if ($user->department)
            ...
            <div class="mt-3 flex flex-wrap items-center gap-2">
                <span class="text-xs text-slate-500 font-medium">รหัสพนักงาน:</span>
                <span id="emp-view" class="text-sm font-mono font-semibold text-slate-300">{{ $user->employee_number ?: '—' }}</span>
                @if ($canInfoEdit)
                <button id="emp-edit-btn" onclick="empEdit()"
                        class="text-xs font-semibold text-indigo-400 hover:text-indigo-300 bg-indigo-500/15 hover:bg-indigo-500/25 px-2 py-0.5 rounded-lg transition-colors cursor-pointer">แก้ไข</button>
                <form id="emp-form" style="display:none"
                    ...
                    <input type="text" name="employee_number" value="{{ $user->employee_number }}" placeholder="เช่น EMP001"
                           class="text-sm bg-white/10 border border-white/20 text-white rounded-lg px-3 py-1.5 w-28 focus:outline-none focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-400/60 transition-all placeholder:text-slate-500">
                    <button type="submit" class="text-xs font-semibold text-white bg-indigo-500/40 hover:bg-indigo-500/60 border border-indigo-400/40 rounded-lg px-3 py-1.5 transition-colors">บันทึก</button>
                    <button type="button" onclick="empCancel()" class="text-xs font-semibold text-slate-400 hover:text-slate-200 hover:bg-white/10 rounded-lg px-2 py-1.5 transition-colors cursor-pointer">ยกเลิก</button>
                </form>
                @endif
            </div>
        </div>
        ...
        <div class="flex items-center gap-2 flex-wrap sm:flex-shrink-0 sm:flex-col sm:items-end">
            <a href="{{ route('users.permission-timeline', $user) }}"
               class="inline-flex items-center gap-1.5 text-xs font-semibold text-violet-300 hover:text-violet-200 bg-violet-500/15 hover:bg-violet-500/25 border border-violet-500/25 rounded-xl px-3 py-2 transition-all">
            ...
            <form action="{{ route('users.import') }}" method="POST">
                ...
                <button type="submit" class="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-300 hover:text-white bg-white/8 hover:bg-white/15 border border-white/15 rounded-xl px-3 py-2 transition-all">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('users.index') }}"
               class="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-400 hover:text-slate-200 hover:bg-white/10 px-3 py-2 rounded-xl transition-colors">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
